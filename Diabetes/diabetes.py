...
def app():
    ...
    if 'logged_in' in st.session_state:
        if not st.session_state.logged_in['is_logged']:
            st.info(":green[Model bạn đang sử dụng có độ chính xác là 67%!] Đăng nhập để sử dụng model chính xác hơn!!")
        else:
        ...
        with col1:
            ...
            input1 = st.text_input(":green[1 - HGB] ")
            input2 = st.text_input(":green[2 - MCHC] ")
            input3 = st.text_input(":green[3 - MCH] ")
            input4 = st.text_input(":green[4 - MCV] ")
            input5 = st.text_input(":green[5 - MPV ] ")
            input6 = st.text_input(":green[6 - RBC  ]")
            input7 = st.text_input(":green[7 - PLT ] ")
            input8 = st.text_input(":green[8 - RDW  ]")
            input9 = st.text_input(":green[9 - WBC ]")
        ...
        if st.button("Dự đoán bệnh"):
            ...
            query.insert_diabetes(user_id, input1, input2, input3, input4, input5, input6, input7, input8, input9,prediction[0])
            if prediction[0] == 1:
